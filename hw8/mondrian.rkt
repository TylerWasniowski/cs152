...
(define (mondrian width height rands)
  (letrec (
           ...
           (split-both (λ (w h rands) (let*
                                          (
                                           (top (split-vert w (top-height h (split-modifier h (first rands))) (rest rands)))
                                           (bottom (split-vert w (bottom-height h (split-modifier h (first rands))) (list* (first (rest rands)) (second top))))
                                           )
                                        (list (vc-append (first top) (first bottom)) (second bottom))
                                        )))
...
(first (mondrian 400 500 (randoms 4901 400)))
